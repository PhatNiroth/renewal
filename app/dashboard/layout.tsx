...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const sessionUser = session?.user as { isAdmin?: boolean; roleName?: string; permissions?: Record<string, { view?: boolean }> } | undefined
  const isAdmin     = sessionUser?.isAdmin === true
  const userRole    = isAdmin ? "Admin" : (sessionUser?.roleName ?? "")
  const initials    = userName.slice(0, 2).toUpperCase()

  const visibleNavItems = navItems.filter(({ module }) => {
    if (!module) return true          // Overview always visible
    if (isAdmin) return true          // Admins see everything
    return sessionUser?.permissions?.[module]?.view === true
  })
